...
public class NBTTooltip implements ClientModInitializer {
    ...
    private static boolean isPressed(MinecraftClient mc, KeyBinding key) {
        return !key.isUnbound() && InputUtil.isKeyPressed(mc.getWindow().getHandle(), InputUtil
                .fromTranslationKey(key.getBoundKeyTranslationKey())
                .getCode());
    }
    ...
    private static NbtCompound encodeStack(ItemStack stack) {
        ...
        NbtCompound tag = (NbtCompound) nbtElement; // cast here, as soon as this breaks, the mod will need to update anyway
        return tag;
    }
    ...
    private static void copyToClipboard(ItemStack stack, MinecraftClient mc) {
        ...
        try {
            ...
            mc
                    .getToastManager()
                    .add(new SystemToast(Type.PERIODIC_NOTIFICATION, Text.translatable("nbttooltip.copied_to_clipboard"), Text.translatable("nbttooltip.object_details", name)));
        } catch (Exception e) {
            mc
                    .getToastManager()
                    .add(new SystemToast(Type.PERIODIC_NOTIFICATION, Text.translatable("nbttooltip.copy_failed"), Text.literal(e.getMessage())));
            e.printStackTrace();
        }
    }
